...
def order(com):
    client.publish('/leds/esp8266',com)
...
cv2.createTrackbar("L B H", "ball", 30, 255, nothing)
...
cv2.createTrackbar("L CF V", "car_front", 60, 255, nothing)
...
while True:
   ...
   # DRAWING LINE OVER OBJECTS
    ...
    if start==True:
        if (not (((ang_car_imgp()>-10 and ang_car_imgp()<10) or (ang_car_imgp()<-170 and ang_car_imgp()>170))) or dis_car_imp()>dis_bb()) and s1==False:
            command = 'R'
            print(command)
            s2=True
            s3=True
            s4=True
        elif (((ang_car_imgp()>-10 and ang_car_imgp()<10) or (ang_car_imgp()<-170 and ang_car_imgp()>170)) or dis_car_imp()<dis_bb())  and s1==False:
            s1=True
            s2=False
            s3=True
            s4=True
            if not (((ang_car_imgp()>-10 and ang_car_imgp()<10) or (ang_car_imgp()<-10 and ang_car_imgp()>170)) or dis_car_imp()<dis_bb()):
                s1=False
                s2=True
                s3=True
                s4=True 
        elif dis_bb()>20 and s2==False:
            command = 'F'
            s3=True
            s4=True
            print(dis_bb(),"dis")
            ...
            s3=False
            s4=True
        if (not (((ang_car_goal()>-10 and ang_car_goal()<10) or (ang_car_goal()<-170 and ang_car_goal()>170))) or dis_car_goal()>dis_cc()) and s3==False:
            command='R'
            print(command)
        elif (((ang_car_goal()>-10 and ang_car_goal()<10) or (ang_car_goal()<-170 and ang_car_goal()>170)) or dis_car_goal()<dis_cc())  and s3==False:
            command='S'
            s3=True
            s4=False
        elif dis_car_goal()>20 and s4==False:
            command='F'
            print(command)
        elif dis_car_goal()<=20 and s4==False:
            command='S'
